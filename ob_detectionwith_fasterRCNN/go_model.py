...
def get_args():
    ...
    parser.add_argument("--video_path", "-v", type=str,default= r"C:\Users\Duong\Downloads\snaptik.vn_37f5a.mp4", help="path to video", required=True)
    parser.add_argument("--saved_checkpoint", "-o", type=str, default=r"C:\Users\Duong\Desktop\python_gui\trained_models\best.pt", help="Load from this checkpoint")
...
def test(args):
    ...
    resuil = cv2.VideoWriter('test.avi', cv2.VideoWriter_fourcc(*'MJPG'), int(cap.get(cv2.CAP_PROP_FPS)), size)
    while cap.isOpened():
        flat , frame = cap.read()
        if not flat:
            break
            ...
            with torch.no_grad():
                ...
                for bbox, label, score in zip(bboxes, labels, scores):
                    if score > args.conf_threshold:
                        ...
                        category = f"{categories[label.item()]}"
